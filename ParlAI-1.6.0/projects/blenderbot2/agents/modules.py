...
DOC_DICT = pickle.load(open('ALL_DOC_DICT','rb'))
#DOC_DICT = {}
...
#         self.linear_cosine = torch.nn.Linear(2560,2560)
#         self.linear_cosine_norm = torch.nn.LayerNorm(2560)
#         self.linear_cosine.weight.data.normal_(0.0,1/np.sqrt(2560))
#         self.linear_cosine.bias.data.fill_(0)
        # attrs
    ...
    def retrieve_and_concat(
        ...
        """
#         print("text_vec:", input)
#         print("memory_decoder_vec:", memory_decoder_vec)
#         print("num_memory_decoder_vecs:",num_memory_decoder_vecs)
#         print("query_generator_vec:",query_generator_vec)
...
#         time.sleep(2)
        ...
        if memory_decoder_vec is not None:
            ...
            for i in range(0,len(generated_memories)):
                if generated_memories[i] == []:
                    generated_memories[i] = ['']
        ...
        if gold_doc_vec is not None:
            search_indices = []
            for idx in range(0,len(gold_doc_vec)):
                if num_gold_docs[idx] > 0:
                    gold_doc_vec_key = str(gold_doc_vec[idx].detach().cpu().tolist())
                    if gold_doc_vec_key in DOC_DICT:
                        search_indices.append(idx)
            search_indices = torch.LongTensor(search_indices)
        
        if search_indices.numel() > 0:
            if gold_doc_vec is None:
                search_docs, search_doc_scores = self.perform_search(
                    search_queries, query_vec, search_indices
                )
            else:
                ### Modify here
                #num_docs = self.opt['n_docs']
                num_docs = 5
                search_docs, search_doc_scores = [], []
                for batch_id in search_indices:
                    gold_doc_vec_key = str(gold_doc_vec[batch_id].detach().cpu().tolist())

                    retrieved_docs   = DOC_DICT[gold_doc_vec_key][0]
                    retrieved_scores = DOC_DICT[gold_doc_vec_key][1].tolist()

                    retrieved_docs  = retrieved_docs[:num_docs]
                    retrieved_docs += [BLANK_DOC] * (num_docs - len(retrieved_docs))

                    retrieved_scores  = retrieved_scores[:num_docs]
                    retrieved_scores += [0.0] * (num_docs - len(retrieved_scores))

                    search_docs.append(retrieved_docs)
                    search_doc_scores.append(retrieved_scores)

                search_doc_scores = torch.Tensor(search_doc_scores).to(input.device)
                dummy_docs = [[BLANK_DOC] * num_docs] * int(n_input)
                dummy_docs_scores = torch.ones(int(n_input), num_docs).to(input.device)

                for j in range(0,len(search_indices)):
                    dummy_docs[search_indices[j]] = search_docs[j]
                    dummy_docs_scores[search_indices[j]] = search_doc_scores[j]

                search_docs = dummy_docs
                search_doc_scores = dummy_docs_scores
                search_indices = torch.LongTensor(range(0,int(n_input))).to(input.device)    
                ### End modify
        ...
        if memory_indices.numel() > 0:
            ...
            if memories is not None and memory_scores is not None:
                ...
                ### Modify here
                if self.long_term_memory.n_docs > 5:
                    dummy_memory = BLANK_DOC
                    dummy_memory_score = 0.0
                    memory_scores = memory_scores.tolist()
                    for i in range(0,len(memories)):
                        count_your_persona = 0
                        for j in range(0,len(memories[i])):
                            memories_text = memories[i][j].get_text()
                            if len(memories_text) <= 2 and j >= 1:
                                memories[i][j] = None
                                memory_scores[i][j] = None
                            elif 'your persona' in memories_text:
                                if count_your_persona >= 2:
                                    memories[i][j] = None
                                    memory_scores[i][j] = None
                                else:
                                    count_your_persona += 1
                        memories[i] = [m for m in memories[i] if m != None][:9]
                        memory_scores[i] = [s for s in memory_scores[i] if s != None][:9]

                    max_num_memories = max([len(m) for m in memories])
                    for i in range(0,len(memories)):
                        for gap in range(len(memories[i]), max_num_memories):
                            memories[i].append(dummy_memory)
                            memory_scores[i].append(dummy_memory_score)

                    memory_scores = torch.Tensor(memory_scores).to(input.device)
                    if input.dtype == torch.float16:
                        memory_scores = memory_scores.half()
                
                self._fill_docs_and_scores(
                    top_docs, doc_scores, memory_indices, memories, memory_scores
                )
#                 for memomry in memories[0]:
#                     print(memomry.get_text())
        ...
        top_doc_scores = torch.stack([torch.cat([s_i for s_i in scores_i]) for scores_i in doc_scores])
    ...
    def perform_search(
        ...
        if self.search:
            ...
            ) or isinstance(self.retriever, SearchQueryFAISSIndexRetriever) or isinstance(
                self.retriever, ObservationEchoRetriever)
...
#             }
        ...
        ### Modify here
        if memory_vec is not None:
            for mem_id in indices:
                if num_memories[mem_id] > 0:
                    memory_dict[mem_id] = memory_vec[mem_id, : num_memories[mem_id]]
        ...
        if self.knowledge_access_method in [
            ...
            for i in range(0,len(memory_dict)):
                new_memory_dict[i] = memory_dict[i]
