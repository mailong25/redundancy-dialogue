...
#         print()
    ...
    def compute_loss(
        ...
        def cal_loss(scores, targets, q_start, q_end, weights = None):
            clamp_min = 1e-5 if self.opt['fp16'] else 1e-5
        ...
        def get_representation(batch, questions):
            foward_model = self.model
            xs = self._model_input(batch)
            ys = batch.label_vec
            ...
            reps = []
            for b_idx in range(0,len(batch.labels)):
                label_questions = questions[b_idx]
                ...
                try:
                    ...
                    print("Error !!", batch.labels[b_idx], label_questions)
                q_start = min((q_end + 1) - len(label_questions_vec) + 1, q_end - 1)
                avg_rep = torch.mean(dec_out[b_idx][q_start:q_end], dim = 0)
                reps.append(avg_rep)
            return reps

        def get_representation_cutoff(batch, questions):
            foward_model = self.model
            xs = self._model_input(batch)
            ys = batch.label_vec
            ...
            # Generate 4 different enc_mask for each sample in batch
            all_reps = []
            
            for each_mask in range(0,4):
                new_mask = deepcopy(enc_mask)
                for i in range(0,len(new_mask)):
                    sent_len = int(new_mask[i].sum().item())
                    mask_len = int(0.3 * sent_len)
                    mask_idx = randint(3, sent_len - 3 - mask_len)
                    new_mask[i][mask_idx : mask_idx + mask_len] = False
                
                dec_out, new_incr_state = foward_model.seq2seq_decoder(
                    inputs, (enc_out, new_mask), None)
                ...
                for b_idx in range(0,len(batch.labels)):
                    ...
                    try:
                        ...
                        q_end = (batch.label_vec[b_idx]==2).nonzero(as_tuple=True)[0][0].item()
                        print("Error !!")
                        print(batch.labels[b_idx])
                        print(label_questions)
                        print()
                    q_start = min((q_end + 1) - len(label_questions_vec), q_end - 1)
                    avg_rep = torch.mean(dec_out[b_idx][q_start:q_end], dim = 0)
                    reps.append(avg_rep)
...
#         print(batch.labels)
        ...
        if negative_questions != None:
            questions = [negative_questions[b_idx][0]['question'] for b_idx in range(0,len(batch.labels))]
            representations = []
            representations.append(get_representation(batch, questions))
            
            ## Cut-off
#             representations += get_representation_cutoff(batch,questions)
#             for i in range(0,5):
...
#             ## Pos Neg            
            ...
            for b_idx in range(0,len(batch.labels)):
                reps = [rep[b_idx] for rep in representations]
                pos_reps = [reps[0], reps[1], reps[2], reps[3], reps[4]]
                neg_reps = [reps[5], reps[6], reps[7], reps[8], reps[9]]
                reps = torch.vstack(pos_reps + neg_reps).type(torch.float32)
                scores = [cosim(reps[i].unsqueeze(0), reps) for i in range(0,10)]
                scores = [torch.exp(s) for s in scores]
                
                total_sample, per_class = 10, 5
                for i in range(0,per_class):
                    demon = 0.0
                    for j in range(0,total_sample):
                        if i != j:
                            demon += scores[i][j]
                    for j in range(0,per_class):
                        if i != j:
                            con_loss += torch.log(scores[i][j]/demon)
            
            con_loss = (-1.0 / 10.0) * con_loss
            con_loss = con_loss / len(batch.labels)
...
#             TEMP_COUNT += 1
#             if TEMP_COUNT % 10 == 0:
#                 print(self.model.linear_cosine.weight)
#                 print(torch.mean(self.model.linear_cosine.weight - old_weight, dim = 1))
#                 print(con_loss)
#                 print()
...
#                                     weights.append(-float('inf'))
                    ...
                    mle_loss = cal_loss(scores, batch.label_vec[b_idx], q_start , q_end, weights)
                    temp = ul_loss.item()
                    ul_loss  = (ul_loss + mle_loss) / 2.0
                    ul_ppl = torch.exp(cal_loss(output_ul, batch.label_vec[b_idx], q_start, q_end))
        ...
        if any([v for v in ul_losses if v != 0]):
            ...
            record_metric('cons_loss', [con_loss] * BATCH_SIZE, BATCH_SIZE)
            num_ul_sample = len([v for v in ul_losses if v != 0])
            loss_ul = loss_ul / float(num_ul_sample)
            loss = loss + con_loss
