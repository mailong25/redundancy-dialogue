...
class TorchGeneratorAgent(TorchAgent, ABC):
    ...
    def _generate(
        ...
        PATH_TO_STATES = './logs/hidden_states/'
        file_name = [f for f in os.listdir(PATH_TO_STATES) if '.pt' in f and '_beam_out_' not in f and 'score' not in f]
        if len(file_name) > 0 and self.opt.get('beam_length_penalty') == 0.6501:
            file_name = sorted(file_name)[-1]
            context_rep = torch.load(PATH_TO_STATES + file_name).to(torch.float32)
        ...
        for _ts in range(max_ts):
            ...
            score = F.softmax(score, dim=-1, dtype=torch.float32)  # type: ignore
            
            ### 1 steps ahead
            
            # Pick the most probable tokens and decoding
            if score.shape[2] in [8008] and self.opt.get('beam_length_penalty') == 0.6501:
                clone_beams = deepcopy(beams)
                clone_score = deepcopy(torch.log(score))
                clone_incr_state = deepcopy(incr_state)
                clone_decoder_input = deepcopy(decoder_input)
                
                for i, b in enumerate(clone_beams):
                    if not b.is_done():
                        b.advance(clone_score[i])

                temp_incr_state_inds = torch.cat(
                    [
                        beam_size * i + b.get_backtrack_from_current_step()
                        for i, b in enumerate(clone_beams)
                    ]
                )
                clone_incr_state = model.reorder_decoder_incremental_state(
                    clone_incr_state, temp_incr_state_inds
                )

                selection = torch.cat(
                    [b.get_output_from_current_step() for b in clone_beams]
                ).unsqueeze(-1)

                clone_decoder_input = self._get_next_decoder_input(
                    clone_decoder_input, selection, temp_incr_state_inds
                )
                
                if self.END_IDX not in clone_decoder_input:
                    with torch.no_grad():
                        dec_out, _ = model.seq2seq_decoder(clone_decoder_input, encoder_states, clone_incr_state)
                        dec_out = dec_out[:, -1, :].to(torch.float32)
                        file_to_save = file_name + '_beam_out_' + str(_ts) + '.pt'
                        torch.save(dec_out, PATH_TO_STATES + file_to_save)
                        cos_scores = [torch.max(cosim(dec_out[i].unsqueeze(0),context_rep)).item() for i in range(0,beam_size)]

                    for i in range(0,beam_size):
                        selected_ids = []
                        for j in range(0,beam_size):
                            if torch.equal(decoder_input[i], clone_decoder_input[j][:-1]):
                                selected_id = clone_decoder_input[j][-1].item()
                                adjust_score = 1 + 0.3*score[0][i][selected_id] - 0.7*cos_scores[j]
                                score[0][i][selected_id] = torch.log(adjust_score)
                                selected_ids.append(selected_id)

                        token_mask = torch.ones_like(score[0][i], dtype=torch.bool)
                        token_mask[selected_ids] = False
                        score[0][i][token_mask] = neginf(score.dtype)

                    torch.save(score, PATH_TO_STATES + 'score' + str(_ts) +'.pt')
                else:
...
class TreeSearch(object):
    ...
    def advance(self, logprobs, d_outs = None):
        """
        ...
        #----Force to generate questions----
        # 38 = ? | 2 = [EOS] | 21 = .
        
        hyp_ids = path_selection.hypothesis_ids
        tok_ids = path_selection.token_ids
        
        q_idxs  = [714, 2334, 1145, 2354, 2122, 749, 1083, 7871, 855, 1140,
                   3055, 946, 1700, 1856, 4806, 4816, 2219, 1279, 5341, 2950,
                   3563, 2849, 2119, 2046, 1586, 976, 2410, 7567, 370]
        p_tokens = [21]
        q_token  = 38
        
#         q_idxs  = [2065, 9026, 1653, 6354, 5342, 858, 4166, 10131, 1378, 1684,
#                    10351, 2145, 7735, 8318, 11165, 7879, 8196, 1741, 24217, 19417,
#                    10362, 2565, 10932, 1707, 4235, 1152, 8924, 15180, 374]
#         p_tokens = [17, 4]
#         q_token = 34
        
        s_token = self.dict.tok2ind[self.dict.start_token]
        e_token = self.dict.tok2ind[self.dict.end_token]
        
        temperature = 1.5     #temperature = 1 (more diverse, off-distribution), temperature = 10 (safe-most-likely)
        topk = 5
        beam_max_length = 40
        beam_min_length = 5
        q_mul = 2.0         #question weight multiplier
        beam_mul = 1.0
        set_seed(42)
        
        if self.length_penalty < 0.65 or self.length_penalty == 0.6501:
            for idx in range(self.beam_size):
                hyp_id = hyp_ids[idx]
                
                if (len(self.partial_hyps[hyp_id]) > beam_max_length or q_token in self.partial_hyps[hyp_id] 
                    or any([qid for qid in q_idxs if qid in self.partial_hyps[hyp_id]])):
                    continue
                
                if self.length_penalty == 0.649:
                    if tok_ids[idx] in q_idxs or any([qid for qid in q_idxs if qid in self.partial_hyps[hyp_id]]):
                        self.scores[idx] = -1000.0
                else:
                    #any([qid for qid in q_idxs if qid in self.partial_hyps[hyp_id]])
                    if self.partial_hyps[hyp_id][-1] not in p_tokens and tok_ids[idx] != self.eos:
                        continue
                    
                    q_scores = torch.Tensor([logprobs[hyp_id][q] for q in q_idxs]) / q_mul
                    
                    ## score is negative
                    if (
                        len(self.partial_hyps[hyp_id]) >= beam_min_length
                        and (torch.max(q_scores).item() > logprobs[hyp_id][tok_ids[idx]] 
                             or (self.length_penalty in [0.648, 6501] and tok_ids[idx] == self.eos)
                            )
                       ):
                        probs = torch.softmax(q_scores * temperature, dim=-1)
                        sprobs, sinds = probs.sort(dim=-1, descending=True)
                        
                        #TopK
                        sprobs[topk:] = 0.0
                        
                        sprobs.div_(sprobs.sum(dim=-1))
                        q_choosen = torch.multinomial(sprobs, 1).item()
                        q_choosen = sinds[q_choosen].item()
                        
                        tok_ids[idx] = q_idxs[q_choosen]
                        self.scores[idx] = self.scores[idx] - logprobs[hyp_id][tok_ids[idx]] + q_scores[q_choosen]

                    if tok_ids[idx] in q_idxs:
                        self.scores[idx] = self.scores[idx] / beam_mul
        
        #----End----
        path_selection.hypothesis_ids = hyp_ids
        path_selection.token_ids = tok_ids
