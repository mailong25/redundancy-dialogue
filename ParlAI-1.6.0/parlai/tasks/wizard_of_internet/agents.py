...
class WizardDialogTeacher(WizardOfInternetBaseTeacher):
    ...
    def teacher_setup_data2(self, datafile) -> Message:
    ...
    def teacher_setup_data(self, datafile) -> Message:
        ...
        for message, episode_started in super().teacher_setup_data(datafile):
            ...
            if episode_started is False:
                message.force_set('text',acc_text + '\n' + message['text'])
            
            acc_text = message['text'] + '\n' + message['labels'][0]
            
            if EXCLUDE_MSC6 is False or (message['text'].split('\n')[-1], message['labels'][0]) not in msc6:
                all_samples.append((message, True))
        
        random.seed(13)
        random.shuffle(all_samples)
        
        print("WOI:", len(all_samples))
