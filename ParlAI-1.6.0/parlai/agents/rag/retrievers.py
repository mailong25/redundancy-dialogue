...
#RagRetriever
class ObservationEchoRetriever(SearchQueryRetriever):
    """
    ...
    def tokenize_query(self, query: str) -> List[int]:
        if query in self._query_ids:
            self._saved_query[self._query_ids[query]] = query
    ...
    def retrieve_and_score(
        ...
        for i in range(0,len(retrieved_docs)):
            batch_query.append(self._saved_query[i])
        
        search_queries = self.generate_search_query(batch_query)
        
        top_docs = []
        top_doc_scores = []
        max_n_docs: int = self.n_docs
        for sq, search_results in zip(search_queries, retrieved_docs):
            if not search_results:
                search_results = self._empty_docs(self.n_docs)
            elif len(search_results) < self.n_docs:
                remain_docs = self.n_docs - len(search_results)
